...
class _WeatherHomePageState extends State<WeatherHomePage> {
  static const _savedCityKey = 'selected_city';
  ...
  Future<void> _initializeAndLoad() async {
    ...
    try {
      ...
      final savedCity = preferences.getString(_savedCityKey);
      if (savedCity != null) {
        city = City.fromJson(jsonDecode(savedCity) as Map<String, dynamic>);
      }
    } catch (_) {
      // A corrupt local preference should never prevent the default forecast.
    }

    if (mounted) {
      await _loadForecast(city);
    }
  }
  ...
  Future<void> _selectCity(City city) async {
    ...
    if (_forecast?.city != city) return;
    try {
      final preferences = await SharedPreferences.getInstance();
      await preferences.setString(_savedCityKey, jsonEncode(city.toJson()));
    } catch (_) {
      // The live forecast remains usable when local persistence is unavailable.
    }
  }
  ...
  Future<void> _loadForecast(City city) async {
    ...
    try {
      ...
      if (!mounted || requestId != _forecastRequestId) return;
      setState(() {
        _forecast = forecast;
        _isLoading = false;
      });
