  ...
  Future<WeatherForecast> fetchForecast(City city) async {
    ...
    try {
      ...
      return WeatherForecast(
        ...
        hourly: _parseHourly(hourly, DateTime.parse(current['time'] as String)),
      ...
      throw WeatherException('날씨 데이터를 불러오는데 실패했습니다: $e');
    }
  }

  List<HourlyWeather> _parseHourly(
    ...
    for (var index = 0; index < times.length; index++) {
      ...
      // Open-Meteo returns local wall-clock times when `timezone=auto` is used.
      // Compare them with the API's current time instead of the device timezone.
